...
*   **Cenário 1 (Python s/ Cache):** Implementação baseada no **Step 3** (Flask).
*   **Cenário 2 (Python c/ Cache):** Implementação baseada no **Step 5** (Flask + Redis).
*   **Cenário 3 (Ruby c/ Cache):** Implementação baseada no **Step 6** (Sinatra + Redis).
*   **Cenário 4 (Ruby s/ Cache):** Versão customizada do Step 6, com a remoção manual do suporte ao Redis para isolar a performance da linguagem.

## 3. Metodologia de Teste
A geração de carga foi realizada via **Locust** em container Docker, orquestrada por um script de automação (`run_scenarios.sh`).

### Configuração do Locust:
*   **Task:** Cada usuário percorre sequencialmente uma lista de 10 URLs (`TARGET_URLS`).
...
O script build_report.py executa o pós-processamento dos dados, validando a integridade do summary.csv e padronizando as métricas para análise. Ele converte o status do cache para formato binário, normaliza as nomenclaturas das linguagens e filtra colunas essenciais de latência e RPS, gerando um relatório consolidado em report.csv.
